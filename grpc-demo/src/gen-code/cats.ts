...
export interface CatsServiceClient {
  findOne(request: ById): Observable<Cat>;

  findMany(request: Pagination): Observable<CatsData>;

  findSome(request: Empty): Observable<CatsData>;
}
...
export interface CatsServiceController {
  ...
  findSome(request: Empty): Promise<CatsData> | Observable<CatsData> | CatsData;
}

export function CatsServiceControllerMethods() {
  return function (constructor: Function) {
    const grpcMethods: string[] = ["findOne", "findMany", "findSome"];
